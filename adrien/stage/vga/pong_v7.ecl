...
let rec next_pos(x,y,dx,dy) =
    ...
    if dx > 0 then
        ...
        let nb_iterx = (x-30)/-dx in (* nb de pas pour atteindre la ligne de la raquette *)
        let nb_itery1 = y/abs(dy) in (* nb de pas pour atteindre le mur du haut *)
        let nb_itery2 = (480-y-ball_size)/abs(dy) in (* nb de pas pour atteindre le mur du bas *)
        if nb_iterx < min(nb_itery1,nb_itery2) then
            y+nb_iterx*dy-player_h/2
        else
            next_pos(x+dx*min(nb_itery1,nb_itery2),y+min(nb_itery1,nb_itery2)*dy,dx,-dy)
    ) ;;

let move((ball,player1,player2,p1_dy,(ball_dx,ball_dy),score) : box * box * box * int<11> * (int<11>*int<11>) * int<5>) : box * box * box * (int<11>*int<11>)* int<5> =
    ...
    let player2_delta(top_col,bot_col) = let (_,dy,pos,_) = reg (fun (old_dy,r_dy,goal,cpt) ->
        (* let (pos,rdy) = exec (next_pos(ball_x,ball_y,ball_dx,ball_dy)) default (0) in *)
        let pos = 0 in
        if cpt = 1000000 then
            let delta = if ball_y + ball_h / 2 > player2_y + player2_h / 2 then 1 else -1 in
            (* let delta = if pos > player2_y then 1 else -1 in *)
            let dy = (if (top_col && delta < 0) or (bot_col && delta > 0) then 0 else delta) in
            (dy,dy,ball_y + ball_h/2,0)
        else
            (old_dy,0,pos,cpt+1)
    ) last (0,0,240,0) in dy,pos in
    ...
    let (player1,player1_dy) =
        ...
        let collision_ball_top = ball_y + ball_h >= player1_y - pot_dy && ball_x + ball_w >= player1_x in
        let collision_ball_bot = ball_y <= player1_y + player_h + pot_dy && ball_x + ball_w >= player1_x in
        
        let top_col = (collision_top_wall or collision_ball_top) in (* -v pour deplacement *)
        let bot_col = (collision_bottom_wall or collision_ball_bot) in (* +v pour deplacement *)
    ...
    let (player2,player2_dy) =
        ...
        let collision_ball_top = ball_y + ball_h >= player2_y - pot_dy && ball_x + ball_w >= player2_x in
        let collision_ball_bot = ball_y <= player2_y + player_h + pot_dy && ball_x + ball_w >= player2_x in
        
        let top_col = (collision_top_wall or collision_ball_top) in (* -v pour deplacement *)
        let bot_col = (collision_bottom_wall or collision_ball_bot) in (* +v pour deplacement *)
        let (dy,pos) = player2_delta(top_col,bot_col) in
        (player2_x,player2_y+dy,player_w,player_h,player_color), if player2_y < pos then 1 else -1
    in

    let (player1_x,player1_y,player1_w,player1_h,(_,_,_)) = player1 in
    let (player2_x,player2_y,player2_w,player2_h,(_,_,_)) = player2 in

    let ball_delta(left_col,right_col,top_col,bot_col,score_delta,col_p1,col_p2) =
        let up(x,y) =
        ...
        in
        let ((ddx,ddy),(dx,dy),_,score) = reg (fun ((old_dx,old_dy),(r_dx,r_dy),cpt,score) ->
            if cpt = 1000000 then (
                let (dx,dy) =
                    if col_p1 && right_col then
                        if player1_dy < 0 then up(old_dx,old_dy) else if player1_dy > 0 then down(old_dx, old_dy) else (old_dx,old_dy)
                    else if col_p2 && left_col then
                        if player2_dy < 0 then up(old_dx,old_dy) else if player2_dy > 0 then down(old_dx, old_dy) else (old_dx,old_dy)
                    else (old_dx,old_dy)
                in
        ...
        in (ddx,ddy,dx,dy,score)
    in
    let (ball,(dx,dy),score) =
        ...
        let collision_player1_right = ball_x + ball_w + pot_dx >= player1_x && (ball_y + ball_h + pot_dy >= player1_y or ball_y - pot_dy <= player1_y + player_h) in
        let collision_player1_top = ball_y - pot_dy <= player1_y + player_h && (ball_x + ball_w + pot_dx >= player1_x) in
        let collision_player1_bot = ball_y + ball_h + pot_dy >= player1_y && (ball_x + ball_w + pot_dx >= player1_x) in
        let collision_player1 = collision_player1_right or collision_player1_top or collision_player1_bot in
        
        let collision_player2_left = ball_x - pot_dx <= player2_x + player_w && (ball_y + ball_h + pot_dy >= player2_y or ball_y - pot_dy <= player2_y + player_h) in
        let collision_player2_top = ball_y - pot_dy <= player2_y + player_h && (ball_x + ball_w + pot_dx >= player2_x) in
        let collision_player2_bot = ball_y + ball_h + pot_dy >= player2_y && (ball_x + ball_w + pot_dx >= player2_x) in
        let collision_player2 = collision_player2_left or collision_player2_top or collision_player2_bot in
        
        let left_col = collision_left_wall or collision_player2_left in
        let right_col = collision_right_wall or collision_player1_right in
        let top_col = collision_top_wall or collision_player1_top or collision_player2_top in
        let bot_col = collision_bottom_wall or collision_player1_bot or collision_player2_bot in
        
        let (ddx,ddy,dx,dy,score) = ball_delta(left_col,right_col,top_col,bot_col,if collision_right_wall then -1 else if collision_left_wall then 1 else 0,collision_player1,collision_player2) in
        
        (ball_x+dx,ball_y+dy,ball_size,ball_size,ball_color),(ddx,ddy),score
    in
    
    (ball,player1,player2,(dx,dy),score) ;;
...
let main(((s0,s1,s2,s3,s4,s5,s6,s7,s8,s9), (k0,k1), gsensor_int, gsensor_sdo, gsensor_sdi) : ((bool*bool*bool*bool*bool*bool*bool*bool*bool*bool) * (bool*bool) * int<2> * std_logic * std_logic)) =
    ...
    let (ball,player1,player2,score,_,_) = reg (fun (ball,player1,player2,score,(dx,dy),pause) ->
        let accel_y = calibrate_accel(not(k0), accel_y) in
        if pause < 50000000 then (ball,player1,player2,score,(dx,dy),pause+1)
        else if s0 then (ball,player1,player2,score,(dx,dy),pause)
        else (
            let player1_dy = control_levels(accel_y) in
            let (b,p1,p2,(dx,dy),s) = move(ball,player1,player2, player1_dy,(dx,dy),score) in
            if score <> s then
                ...
                (ball,player1,player2,s,(dx,dy),0)
            else
                (b,p1,p2,s,(dx,dy),pause)
        )
    ) last (
        let player1 = (610,190,player_w,player_h,(-1,-1,-1)) in
        let player2 = (10,190,player_w,player_h,(15,15,15)) in
        let ball = (313,233,ball_size,ball_size,(-1,0,0)) in (* 313,233 *)
        (ball,player1,player2,0,(2,1),-250000000)
    ) in
