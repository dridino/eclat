...
let move((ball,player1,player2,p1_dy,score) : box * box * box * int<11> * int<5>) : box * box * box * int<5> =
    ...
    let ball_delta(left_col,right_col,top_col,bot_col,score_delta) =
        ...
        in (dx,dy,score)
    in
    let player1_delta(top_col,bot_col) = let (_,dy,_) = reg (fun (dy,r_dy,cpt) -> if p1_dy = 0 then (dy,0,cpt) else if cpt = 400000 then let dy = (if (top_col && p1_dy < 0) or (bot_col && p1_dy > 0) then 0 else p1_dy) in (dy,dy,0) else (dy,0,cpt+1)) last (0,0,0) in dy in
