...
let ty_op externals op =
  ...
  | Bool_of_std_logic ->
      (T_const TStd_logic) ==> (T_tuple[tbool; tbool])
  | Std_logic_of_bool ->
      tbool ==> (T_const TStd_logic)
  | External_fun e ->
    let rec is_valid = function
    | T_const TBool | T_const (TInt _) | T_const TStd_logic -> true
    | T_tuple l -> List.for_all is_valid l
    | _ -> false
    in
    (match SMap.find_opt e (snd externals) with
    | Some (T_fun{arg;dur=T_response_time 0;ret} as t) -> (
        if ret = T_const TUnit then raise (Invalid_argument "externals must be functions, not procedures, they can't return unit")
        else if not @@ is_valid arg then raise (Invalid_argument "externals can only take arguments of type bool, int, std_logic and a tuple of those")
        else t
    )
    | Some (T_fun{arg;dur;ret}) -> raise (Invalid_argument "external functions must be immediate, consider changing \"->\" by \"=>\"")
    | None -> Prelude.Errors.raise_error ~msg:("unbound external function "^e) ()
    | _ -> assert false)
      


(* improved typing with level of types *)
let ty_op2 externals op =
  ...
  | Std_logic_of_bool ->
      Ty_fun(Ty_base TyB_bool, Dur_zero, TyB_std_logic)
  | Bool_of_std_logic ->
      Ty_fun(Ty_base TyB_std_logic, Dur_zero, TyB_tuple[TyB_bool; TyB_bool])
  | External_fun x ->
        let rec auxA = function
        | Types.T_tuple l -> Ty_tuple (List.map auxA l)
        ...
        | Types.T_tuple l -> TyB_tuple (List.map auxB l)
        | Types.T_const Types.TUnit -> TyB_unit
        | Types.T_const (Types.TInt (Types.T_size s)) ->TyB_int (Sz_lit s)
        | Types.T_const Types.TBool -> TyB_bool
        | Types.T_const Types.TStd_logic -> TyB_std_logic
        | _ -> assert false
        in
        let rec is_valid arg =
            let open Types in
            match arg with
            | T_const TBool | T_const (TInt _) | T_const TUnit | T_const TStd_logic -> true
            | T_tuple l -> List.for_all is_valid l
            | _ -> false
        in
        (match Types.SMap.find_opt x (snd externals) with
        | Some (Types.T_fun{arg;dur=T_response_time 0;ret}) ->
            if ret = T_const TUnit then raise (Invalid_argument "externals must be functions, not procedures, they can't return unit")
            else if not @@ is_valid arg then raise (Invalid_argument "externals can only take type bool, int, std_logic and a tuple of those")
            else Ty_fun(auxA arg, Dur_zero, auxB ret)
        | Some (T_fun{arg;dur;ret}) -> raise (Invalid_argument "external functions must be immediate, consider changing \"->\" by \"=>\"")
        | None -> Prelude.Errors.raise_error ~msg:("unbound external function "^x) ()
        | _ -> assert false)
...
let pp_op fmt (op:op) : unit =
  ...
  | Std_logic_of_bool -> "std_logic_of_bool"
  | Bool_of_std_logic -> "bool_of_std_logic"
  | External_fun x -> Printf.sprintf "external(%s)" x
...
let gen_op fmt (op:op) pp a : unit =
  ...
  | Std_logic_of_bool ->
      procall fmt "eclat_std_logic_of_bool"
  | Bool_of_std_logic ->
      procall fmt "eclat_bool_of_std_logic"
  | External_fun x -> procall fmt (Printf.sprintf "work.%s" x)
