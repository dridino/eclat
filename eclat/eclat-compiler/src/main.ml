...
let main () : unit =
  ...
  if !typing_with_tyB then
    (let _,_ = Typing.Typing2.typing ~collect_sig:false ~statics:pi.statics ~sums:[] ~externals pi.main in
     ());
  ...
  Gen_testbench.gen_testbench fmt_tb ~vhdl_comment typing_env name ty (argument,result) args ~externals;
