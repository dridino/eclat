...
| Ast.Bool b -> Bool b
| Ast.Std_logic s -> Std_logic (match s with ZERO -> ZERO | ONE -> ONE | Z -> Z)
...
let rec to_s ~externals ~statics ~sums gs e x k =
  let return_ s =
    seq_ s k
  in
  ...
  | Ast.E_if(a,e1,e2) ->
       ...
       *)
      (* ************************** *)
      let w1,ts1,s1 = to_s ~externals ~statics ~sums gs e1 x k in
      let w2,ts2,s2 = to_s ~externals ~statics ~sums gs e2 x k in
      let z = Ast.gensym () in
  ...
  | E_case(a,hs,e_els) ->
      ...
      then
        ...
        let k_cut = S_skip in
        let w,ts,s = to_s ~externals ~statics ~sums gs e x k_cut in
        assert (SMap.is_empty ts);
        w,ts,seq_ s k
        (* ************************** *)  
      else
      let ws,tss,hs' = Prelude.map_split3 (fun (c,e) ->
                         let w,ts,s = to_s ~externals ~statics ~sums gs e x k in
                         w,ts,(to_c c,s)
                       ) hs
      in
      let ts = List.fold_left (++) SMap.empty tss in
      let w1,ts1,s1 = to_s ~externals ~statics ~sums gs e_els x k in
  ...
  | E_match(a,hs,eo) ->
      ...
      let wn,tsn,so = match eo with
                      ...
                      | Some e -> let w,ts,s = to_s ~externals ~statics ~sums gs e x k in
                                  (w,ts,Some s)
      in
  ...
  | E_letIn(P_var f,(E_fix(h,(p,e1)) as phi),e2) ->
     ...
     let w1,ts1,s1 = to_s ~externals ~statics ~sums (f::gs) e1 (NameC.result_of_fun f) (S_continue f') in
     let w2,ts2,s2 = to_s ~externals ~statics ~sums gs e2 x k in
  ...
  | E_letIn(P_unit,e1,e2) ->
      ...
      let w1,ts1,s1 = to_s ~externals ~statics ~sums gs e1 (Ast.gensym ()) s2 in
      w1++>w2,ts2++ts1,s1
  | E_letIn(P_var y,e1,e2) ->
      ...
      if Combinational.combinational e1 then
        ...
        let w1,ts1,s1 = to_s ~externals ~statics ~sums gs e1 y s2 in
        w1++>w2,ts2++ts1,s1
  | E_app(E_var f,a) ->
  ...
  | E_set(ay,a) ->
      ...
      (match ay with
       | E_var y ->
          ...
          let w,ts,s = to_s ~externals ~statics ~sums gs (E_const Unit) x k in
          let s0 = S_if(y^"_lock", 
  ...
  | E_reg((p,e1),e0,_) ->
      let y = match p with 
              ...
              | _ -> assert false 
      in
      let pi = Ast.{externals;statics;sums;main=e1} in
  ...
  | E_par(es) ->
      ...
      let pi_s = List.map (fun e -> compile @@ Ast.{externals;statics;sums;main=e}) es in
  ...
  | E_run(y,e) ->
    ...
    begin
      match SMap.find_opt y (fst externals) with
      | Some(_,shared) ->
          if shared then
            ...
            let q1 = Ast.gensym ~prefix:"pause_externalI" () in
            let ts = SMap.add q1 (seq_ (S_start(y,false)) k) SMap.empty in 
            let s = seq_ (S_set_arg(y,args)) @@
                    seq_ (S_start(y,true)) @@
                    seq_ (set_ x (A_get_result(y))) @@
                    seq_ (S_ptr_take(y,false)) (S_continue q1) in
            let q_wait = Ast.gensym ~prefix:"q_wait" () in
            let s' = let_plug_s (A_ptr_taken(y)) @@ fun z ->
                      S_if(z, (S_continue q_wait),
                              Some (seq_ (S_ptr_take(y,true)) s)) in
            ...
            let t1 = seq_ (S_set_arg(y,args)) @@
                      (* seq_ ( *)return_ (set_ x (A_get_result(y)))(* ) k *)
            in
...
and compile ?(result=(Ast.gensym ~prefix:"result" ())) pi =
  ...
  let w0,ts0,s0 = to_s ~externals ~statics ~sums [compute] pi.main x k in
