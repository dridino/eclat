...
and ty = TInt of ty | TBool | TUnit | TStd_logic
       | TTuple of ty list
       | TVar of tvar ref
       | TString of ty
...
module Debug = struct
  ...
  | TStd_logic -> fprintf fmt "std_logic"
...
let pp_vector = Ast_pprint.pp_vector
  ...
  let rec pp_c fmt = function
    ...
    | Std_logic s -> let s = match s with | ONE -> "ONE" | ZERO -> "ZERO" | Z -> "Z" in fprintf fmt "%s" s
  ...
  | A_get_result(x) -> fprintf fmt "%s_result_%d" x !get_res_ref; incr get_res_ref
  ...
  | S_set_arg(x,a) ->
    fprintf fmt "@[%s_argument_%d <= %a;@]" x !set_arg_ref pp_a a; incr set_arg_ref
  | S_start(x,b) ->
    fprintf fmt "@[%s_start_%d <= %s;@]" x !start_ref (if b then "'1'" else "'0'"); incr start_ref
