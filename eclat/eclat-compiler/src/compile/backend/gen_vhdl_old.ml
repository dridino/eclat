...
and pp_call externals fmt (op,a) =
  ...
  | Runtime(External_fun x) -> (
      ...
      let type_conv t s=
        ...
        | T_const (TInt _i) -> Printf.sprintf "signed(%s)" s
        | T_const (TBool) -> Printf.sprintf "%s(0)" s
        | T_const (TStd_logic) -> Printf.sprintf "%s(0)" s
        | _ -> s
      in
...
| A_is_rdy(x) -> fprintf fmt "%s_rdy" x
| A_get_result(x) -> fprintf fmt "%s_result" x
...
| S_fsm(id,rdy,x,cp,ts,s,b) ->
     ...
     pp_fsm fmt ~restart:b ~externals ~state_var:st2 ~compute:cp ~rdy (id,ts,s)
| S_in_fsm(id,s) ->
     let (st2,_,_) = List.assoc id !List_machines.extra_machines in
...
| S_set_arg(x,a) ->
  fprintf fmt "@[%s_argument <= %a;@]" x (pp_a externals) a
| S_start(x,b) ->
  fprintf fmt "@[%s_start <= %s;@]" x (if b then "'1'" else "'0'")
...
let declare_variable ~argument ~externals ~statics typing_env fmt =
  ...
  Types.SMap.iter (fun i _ -> add_var (Printf.sprintf "$%s_ptr_take" i) 1) (fst externals);
...
*)
  ...
  let print_external n ty =
    ...
    fprintf fmt "signal %s_argument : std_logic_vector(0 to %d);\n" n (arg - 1);
    fprintf fmt "signal %s_rdy : std_logic_vector(0 to 0);\n" n;
    fprintf fmt "signal %s_result : std_logic_vector(0 to %d);\n" n (ret - 1);
  ...
  pp_fsm ~restart:false fmt ~state_var ~compute ~externals ~rdy ("main",ts,s);
