...
let get_res_ref = ref 0
let is_rdy_ref = ref 0

(** code generator for tuples deconstruction *)
let rec pp_tuple_access fmt externals (i:int) ty (a:a) : unit =
...
and pp_call externals fmt (op,a) =
  match op with
  | GetTuple(i,_,ty) -> pp_tuple_access fmt externals i ty a
  ...
  | Runtime(Vector_make) ->
      (match a with
      | A_tuple[a1; a2] -> fprintf fmt "eclat_vector_make(%a,%a)" (pp_a externals) a1 (pp_a externals) a2
      | _ -> assert false)
  | Runtime(Vector_length (sz,sz_res)) ->
   ...
   | Runtime(Vector_get t) ->
      fprintf fmt "eclat_vector_get(%a,%d)" (pp_a externals) a Fsm_typing.(size_ty (translate_ty t))
   | Runtime(Vector_update t) ->
      fprintf fmt "eclat_vector_update(%a,%d)" (pp_a externals) a Fsm_typing.(size_ty (translate_ty t))
  | Runtime(External_fun x) -> (
    let rec arg_type_list arg =
      let open Types in
      match arg with
      | T_const TBool | T_const (TInt _) | T_const TUnit | T_const TStd_logic -> [arg]
      | T_tuple l -> List.concat (List.map arg_type_list l)
      | _ -> assert false
    in
    let rec parse_args arg =
      match arg with
      | A_const Unit -> []
      | A_const (Bool _ as c) | A_const (Int _ as c) | A_const (Std_logic _ as c) ->
        [(let f1 = str_formatter in pp_c f1 c; flush_str_formatter ())]
      | A_const CTuple l -> List.map (fun c -> let f1 = str_formatter in pp_c f1 c; flush_str_formatter ()) l
      | A_tuple l -> List.concat (List.map parse_args l)
      | A_var v -> [(let f1 = str_formatter in fprintf f1 "%a" pp_ident v; flush_str_formatter ())]
      | _ -> [(let f1 = str_formatter in fprintf f1 "%a" (pp_a externals) arg; flush_str_formatter ())]
    in
    let type_conv t s=
      let open Types in
      match t with
      | T_const (TUnit) -> Printf.sprintf ""
      | _ -> s
    in
    match Types.SMap.find_opt x (snd externals) with
    | None -> assert false (* can't happen *)
    | Some Types.T_fun{arg;dur;ret} ->
      let formatted_s = let tmp = parse_args a in if tmp = [] then "" else let l = List.map2 type_conv (arg_type_list arg) (parse_args a) in Printf.sprintf "(%s)" @@ List.fold_left (fun acc s -> acc ^ ", " ^ s) (List.hd l) (List.tl l) in
      (match ret with
      | Types.T_const Types.TStd_logic | Types.T_const Types.TBool -> fprintf fmt "work.%s%s" x formatted_s
      | _ -> fprintf fmt "std_logic_vector(work.%s%s)" x formatted_s)
    | _ -> assert false
)
...
| A_is_rdy(x) ->
  let res = match Types.SMap.find_opt x (fst externals) with
  | Some (_,shared) -> if shared then fprintf fmt "%s_rdy" x else fprintf fmt "%s_rdy_%d" x !is_rdy_ref
  | _ -> assert false in
  incr is_rdy_ref;
  res
| A_get_result(x) ->
  let res = match Types.SMap.find_opt x (fst externals) with
  | Some (_,shared) -> if shared then fprintf fmt "%s_result" x else fprintf fmt "%s_result_%d" x !get_res_ref
  | _ -> assert false in
  incr get_res_ref;
  res


let set_arg_ref = ref 0

(** code generator for statements *)
let rec pp_s ~st externals fmt = function
| S_skip -> ()
| S_continue q -> fprintf fmt "%a <= %a;" pp_ident (st^"%next") pp_ident q
| S_if(z,s1,so) ->
...
| S_case(y,hs,so) ->
    ...
    List.iter (fun (c,s) -> fprintf fmt "@[<v 2>when %a =>@,%a@]@," pp_c c (pp_s externals ~st) s) hs;
    Option.iter (fun s ->
...
| S_setptr_read(x,idx) -> (* todo: avoid code duplication between S_setptr & S_setptr_write *)
    ...
    | _ ->
       fprintf fmt
         "@[%a <= to_integer(unsigned(%a));@]" pp_ident ("$"^x^"_ptr") (pp_a externals) idx)
| S_setptr_write(x,idx,a) ->
    ...
    | _ ->
       fprintf fmt
         "@[%a <= to_integer(unsigned(%a));@]@," pp_ident ("$"^x^"_ptr_write") (pp_a externals) idx);
    fprintf fmt
         "@[%a <= '1';@]@," pp_ident ("$"^x^"_write_request");
    fprintf fmt
      "@[%a <= %a;@]" pp_ident ("$"^x^"_write") (pp_a externals) a;
      ...
      | _ ->
         fprintf fmt
           "@[%a <= to_integer(unsigned(%a));@]" 
            pp_ident (ptr_matrix_read x i) (pp_a externals) idx) (List.rev idx_list)
| S_setptr_matrix_write(x,idx_list,a) ->
    List.iteri (fun i idx ->
      ...
      | _ ->
         fprintf fmt
           "@[%a <= to_integer(unsigned(%a));@]@," 
              pp_ident (ptr_matrix_write x i) (pp_a externals) idx) (List.rev idx_list);
    fprintf fmt "@[%a <= '1';@]@," 
      pp_ident ("$"^x^"_write_request");
    fprintf fmt "@[%a <= %a;@]" pp_ident ("$"^x^"_write") (pp_a externals) a;
| S_ptr_take(x,b) ->
    fprintf fmt
...
| S_fsm(id,rdy,x,cp,ts,s,b) ->
     ...
     pp_fsm ~externals fmt ~restart:b ~state_var:st2 ~compute:cp ~rdy (id,ts,s)
| S_in_fsm(id,s) ->
     let (st2,_,_) = List.assoc id !List_machines.extra_machines in
...
| S_set_arg(x,a) ->
  let res = match Types.SMap.find_opt x (fst externals) with
  | Some (_,shared) -> if shared then fprintf fmt "@[%s_argument <= %a;@]" x (pp_a externals) a else fprintf fmt "@[%s_argument_%d <= %a;@]" x !set_arg_ref (pp_a externals) a
  | _ -> assert false in
  incr set_arg_ref;
  res
| S_start(x,b) ->
  let res = match Types.SMap.find_opt x (fst externals) with
  | Some (_,shared) ->
    if shared then fprintf fmt "@[%s_start <= %s;@]" x (if b then "'1'" else "'0'")
  | _ -> assert false in
  res

(** code generator for FSMs *)
and pp_fsm fmt ~restart ~externals ~state_var:st ~compute ~rdy (id,ts,s) =
    if restart then (
      fprintf fmt "@[<v>case %a is@," pp_ident st;
      List.iter (fun (x,s) -> fprintf fmt "@[<v 2>when %a =>@,%a@]@," pp_ident x (pp_s externals ~st) s) ts;
      fprintf fmt "@[<v 2>when %a =>@,%a@]" pp_ident compute (pp_s externals ~st) s;
      fprintf fmt "@]@,end case;")
    else (
    fprintf fmt "@[<v>case %a is@," pp_ident st;
    List.iter (fun (x,s) -> fprintf fmt "@[<v 2>when %a =>@,%a@]@," pp_ident x (pp_s externals ~st) s) ts;
...
let declare_state_var fmt state_var compute xs =
  let state_var_tname = Naming_convention.state_var_type state_var in
    ...
    fprintf fmt ");@,signal %a, %a: %a;@," pp_ident state_var pp_ident (state_var^"%next") pp_ident state_var_tname
...
let declare_variable ~argument ~externals ~statics typing_env fmt =
  ...
  Types.SMap.iter (fun x (_,shared) -> if shared then add_var ("$" ^ x ^ "_ptr_take") 1) (fst externals);
...
let pp_component fmt ~vhdl_comment ~externals ~name ~state_var ~argument ~result ~compute ~rdy ~statics typing_env infos (ts,s) =
...
*)
  ...
  let print_external n (ty,shared) =
    let rec ty_size ty =
      let open Types in
      match ty with
      | T_const TBool | T_const TUnit | T_const TStd_logic -> 1
      | T_const (TInt i) -> (match i with | T_size i -> i | _ -> assert false)
      | T_tuple l -> List.map ty_size l |> List.fold_left (+) 0
      | _ -> assert false
    in
    let arg,ret = match ty with
    | Types.T_fun{arg;dur;ret} -> ty_size arg, ty_size ret
    | _ -> assert false
    in
    let nb = match Hashtbl.find_opt Ast.external_count n with None -> 0 | Some v -> v in
    if shared then
      (fprintf fmt "signal %s_start : std_logic;\n" n;
      fprintf fmt "signal %s_argument : std_logic_vector(0 to %d) := (others => '0');\n" n (arg - 1);
      fprintf fmt "signal %s_rdy : std_logic_vector(0 to 0) := \"0\";\n" n;
      fprintf fmt "signal %s_result : std_logic_vector(0 to %d) := (others => '0');\n" n (ret - 1))
    else
      for i=0 to nb-1 do
    ...
    fprintf fmt "component %s is\n port(\n" n;
    fprintf fmt "signal clk : in std_logic;\n";
    fprintf fmt "signal reset : in std_logic;\n";
    fprintf fmt "signal start : in std_logic;\n";
    fprintf fmt "signal argument : in std_logic_vector(0 to %d);\n" (arg - 1);
    (* fprintf fmt "signal rdy : out std_logic_vector(0 to 0);\n"; *)
    fprintf fmt "signal result : out std_logic_vector(0 to %d)\n" (ret - 1);
    fprintf fmt ");\nend component;\n\n"
  in
  Types.SMap.iter print_external (fst externals);

  let variables = List.filter (fun (x,t) ->
          x <> argument && not (List.mem_assoc x statics))
    @@ List.of_seq (Hashtbl.to_seq typing_env) in
  let variables = variables @ List.map (fun (x,_) -> ptr_taken x, TBool) statics @ Types.SMap.fold (fun x (t,shared) acc -> if shared then (ptr_taken x, TBool) :: acc else acc) (fst externals) [] in


  List.iter (fun (x,t) ->
    if x <> argument then
    let n = size_ty t - 1 in
    fprintf fmt "signal %a : value(0 to %d) := (others => '0');@," pp_ident (x^"%now") n;
    fprintf fmt "signal %a : value(0 to %d) := (others => '0');@," pp_ident (x^"%next") n) (* default value added to make registers work properly *)
  variables;

  (* Types.SMap.iter (fun x (_,shared) ->
    if shared then
      (fprintf fmt "signal %a : value(0 to 0) := (others => '0');@," pp_ident (ptr_taken x ^ "%now");
      fprintf fmt "signal %a : value(0 to 0) := (others => '0');@," pp_ident (ptr_taken x ^ "%next"))
  ) (fst externals); *)

  fprintf fmt "@,@[<v 2>begin@,";

  let instantiate_external n (_,shared) =    
    let nb = match Hashtbl.find_opt Ast.external_count n with None -> 0 | Some v -> v in
    if shared then
      (fprintf fmt "%s_cc : component %s port map (\n" n n;
      fprintf fmt "clk => clk,\n";
      fprintf fmt "reset => reset,\n";
      fprintf fmt "start => '%s_start,'\n" n;
      fprintf fmt "argument => %s_argument,\n" n;
      (* fprintf fmt "rdy => %s_rdy,\n" n; *)
      fprintf fmt "result => %s_result\n" n;
      fprintf fmt ");\n\n")
    else
      for i=0 to nb - 1 do
        fprintf fmt "%s_cc_%d : component %s port map (\n" n i n;
        fprintf fmt "clk => clk,\n";
        fprintf fmt "reset => reset,\n";
        fprintf fmt "start => '1',\n";
        fprintf fmt "argument => %s_argument_%d,\n" n i;
        (* fprintf fmt "rdy => %s_rdy,\n" n; *)
        fprintf fmt "result => %s_result_%d\n" n i;
        fprintf fmt ");\n\n"
      done
  in
  Types.SMap.iter instantiate_external (fst externals);
  ...
  in
  List.iter (fun (x,t) -> set_def x t) variables;
  (* Types.SMap.iter (fun x (_,shared) -> if shared then set_def x TBool else ()) (fst externals); *)

  List.iter (fun (_,(sv,cp,xs)) -> 
      fprintf fmt "%a <= %a;@," pp_ident sv pp_ident cp) !List_machines.extra_machines;


fprintf fmt "elsif (rising_edge(clk)) then@,";
  ...
  let update x =
    fprintf fmt "%a <= %a;@," 
       ...
       pp_ident (x^"%next")
  in
  List.iter (fun (x,_) -> update x) variables;
  (* Types.SMap.iter (fun x (_,shared) -> if shared then update (ptr_taken x) else ()) (fst externals); *)

  List.iter (fun (_,(sv,cp,xs)) -> fprintf fmt "%a <= %a;@," pp_ident sv pp_ident (sv^"%next")) !List_machines.extra_machines;
 ...
 fprintf fmt "%a <= %a;@ @," 
  ...
  fprintf fmt "@[<v 2>process(%a" pp_ident argument;
  if !Operators.flag_no_print then () else fprintf fmt ", clk";
  List.iter (fun (_,(sv,_,_)) -> fprintf fmt ",%a" pp_ident sv) !List_machines.extra_machines;

  List.iter (fun (x,_) -> fprintf fmt ", %a" pp_ident (x^"%now")) variables;
  fprintf fmt ")@,";

  declare_variable ~argument ~externals ~statics typing_env fmt;
  ...
  (* fprintf fmt "@[<hov>";
   Hashtbl.iter (fun x t ->
      ...
      | None ->
          if x <> argument then
            fprintf fmt "%a <= %s;@ @," pp_ident x (default_zero t)
    ) typing_env;
  fprintf fmt "@]";*)
...
*)
  ...
  pp_fsm ~restart:false ~externals fmt ~state_var ~compute ~rdy ("main",ts,s);

  let set_new x =
    fprintf fmt "%a <= %a;@," pp_ident (x^"%next") pp_ident x
  in

  List.iter (fun (x,_) -> set_new x) variables;
  (* Types.SMap.iter (fun x (_,shared) -> if shared then set_new (ptr_taken x) else ()) (fst externals); *)
