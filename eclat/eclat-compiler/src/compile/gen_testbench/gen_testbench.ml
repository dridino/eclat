...
let gen_testbench fmt ?(vhdl_comment="") typing_env name ty ((argument,ta),(result,tr)) (args_for_simul: _ list) ~externals =
...
architecture tb of tb_%s is
  ...
  Gen_vhdl.declare_variable ~argument:"tb_argument" ~externals ~statics:[] typing_env_tb fmt;
  ...
  List.iter (fun s_arg ->
    fprintf fmt "      @[<v>%a@]@," (Gen_vhdl.pp_s ~st:"fake" externals) s_arg;
