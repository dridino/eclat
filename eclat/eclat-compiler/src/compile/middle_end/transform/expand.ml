...
*)
  ...
  | E_vector_first_where(_is_par,(p,e1),e2,ty) ->
    has_changed := true;
    let e1' = expand ~statics @@ e1 in
    (match Types.canon ty with
    | T_size n ->
        ...
        let y = gensym () in
        E_letIn(P_var y,e2,
          let rec loop i finali =
          if i >= n then
            (
              match finali with
              | None -> (E_tuple [E_const (Bool false);E_app(E_const(Op(Runtime (Vector_get (Types.unknown())))), E_tuple[E_var y;E_const(Int(0,Types.T_size 32))])])
              | Some v -> (E_tuple [E_const (Bool true);E_app(E_const(Op(Runtime (Vector_get (Types.unknown())))), E_tuple[E_var y;E_var v])])
            )
          else
          let z = gensym () in
          let x = gensym () in
          let finali_var = gensym () in
          let ii = E_const(Int(i,Types.unknown())) in
          E_letIn(P_var x, E_letIn(P_var z,
                                E_app(E_const(Op(Runtime (Vector_get (Types.unknown())))),
                                E_tuple[E_var y;ii]),
                            Ast_subst.subst_p_e p (E_tuple[ii;E_var z]) (Ast_rename.rename_e ~statics e1')),
          match finali with
          | None -> E_if (E_var x, E_letIn(P_var finali_var, ii, loop (i+1) (Some finali_var)), loop (i+1) finali)
          | Some v -> loop (i+1) finali
          )
          in loop 0 None)
          (* Ast_pprint.pp_exp Format.std_formatter e0; *)
    | _ -> assert false (* todo error *)
