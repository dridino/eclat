...
let compile ?(vhdl_comment="") ?(prop_fsm=false) arg_list name ty fmt pi =
  ...
  let typing_env = Fsm_typing.typing_circuit ~externals ~statics ty (rdy,result,fsm) in
  ...
  let (argument,result) = Gen_vhdl.pp_component fmt ~vhdl_comment ~externals ~name ~state_var ~argument ~result ~compute ~rdy ~statics typing_env (let infos = SMap.empty in infos) fsm in
