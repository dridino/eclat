...
let frontend ~(inputs : string list) repl ?(when_repl=(fun _ -> ())) ?(relax=false) main str_arg : pi * e list =
  let ess_from_files,gss_from_files,tss_from_files,dss_from_files =
    Prelude.map_split4 (fun path ->
                ...
                begin try
                      ...
                      let es,gs,ts,ds = Parser.pi Lexer.token lexbuf in
                      close_in ic;
                      es,gs,ts,ds) lexbuf
                    with excp ->
                      close_in_noerr ic;
  ...
  let es_from_files,gs_from_files,ts_from_files, ds_from_files =
    let e1,e2 = List.split ess_from_files in
    (List.concat e1,List.concat e2), List.concat gss_from_files, List.concat tss_from_files, List.concat dss_from_files in
  ...
  *)
  ...
  ({externals=es_from_files; statics=gs_from_files;sums=ts_from_files;main}, values_list)
