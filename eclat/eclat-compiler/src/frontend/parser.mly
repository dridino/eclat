...
%token OF
%token LET REC AND IN IF THEN ELSE FIX EXTERNAL SHARED RUN
%token <string> IDENT UP_IDENT TVAR_IDENT EXTERNAL_IDENT
%token <bool> BOOL_LIT
%token <int> INT_LIT
...
%token VECTOR_MAPI INT_MAPI VECTOR_FIRST_WHERE
...
%start <((x * ty * bool) list * (x * ty) list) * (x * static) list * (x * (x * ty) list) list * ((p*e)*Prelude.loc) list> pi
...
pi:
| ec=ext_circ pi=pi   { let (ecs,efs),gs,ts,ds = pi in ((ec::ecs,efs), gs,    ts,    ds   ) }
...
ext_fun:
| EXTERNAL x=EXTERNAL_IDENT COL t=fun_ty SEMI_SEMI { (x, t) }

ext_circ:
| EXTERNAL x=UP_IDENT COL t=fun_ty SEMI_SEMI { (x, t, false) }
| SHARED EXTERNAL x=UP_IDENT COL t=fun_ty SEMI_SEMI { (x, t, true) }
...
| RUN i=UP_IDENT LPAREN e=exp RPAREN { Hashtbl.replace external_count i (match Hashtbl.find_opt external_count i with None -> 1 | Some v -> v+1); E_run(i, e) }
...
| x=IDENT { match x with
            ...
            | "std_logic_of_bool" -> E_const (Op(Runtime(Std_logic_of_bool)))
            | "bool_of_std_logic" -> E_const (Op(Runtime(Bool_of_std_logic)))
...
| x=EXTERNAL_IDENT { E_const (Op(Runtime(External_fun x))) }
| VECTOR_FIRST_WHERE e=aexp {
                match Ast_undecorated.remove_deco e with
                | E_tuple[v;e] -> assert (is_variable v || evaluated v); (* todo: error *)
                                  let x = Ast.gensym () in
                                  E_vector_first_where(false,(P_var x,E_app(v,E_var x)), e, unknown ())
                | _ -> assert false (* todo error *) }
