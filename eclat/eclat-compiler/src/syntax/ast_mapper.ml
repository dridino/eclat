...
let rec map f e =
  ...
  | E_run(i,e) ->
      E_run(i, f e)
  | E_vector_first_where (is_par,(p,e1),e2,ty) ->
      E_vector_first_where (is_par,(p,f e1),f e2,ty)
(** traversal order of sub-expressions is unspecified *)
...
let rec iter f (e:e) : unit =
  ...
  | E_int_mapi(_,(_,e1),e2,_) ->
      f e1; f e2
  | E_run (_,e) -> f e
  | E_vector_first_where (_,(_,e1),e2,_) ->
      f e1; f e2
  ...
  let rec aux e =
    ...
    | None ->
        ...
        | E_vector_first_where(is_par,(p,e1),e2,ty) ->
            let ds1,e1' = aux e1 in
            let ds2,e2' = aux e2 in
            ds1@ds2,E_vector_first_where(is_par,(p,e1'),e2',ty)
  ) 
  in 
  aux e
