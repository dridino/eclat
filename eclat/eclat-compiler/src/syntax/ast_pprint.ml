...
let pp_ty (fmt:fmt) (ty:ty) : unit =
  ...
  | T_const tc ->
      (match tc with
       ...
       | TStd_logic -> fprintf fmt "%s" "std_logic")
...
let rec pp_const (fmt:fmt) (c:c) : unit =
  ...
  | Std_logic s ->
      begin
        match s with
        | ZERO -> fprintf fmt "std_logic(ZERO)"
        | ONE -> fprintf fmt "std_logic(ONE)"
        | Z -> fprintf fmt "std_logic(Z)"
      end
...
let pp_exp (fmt:fmt) (e:e) : unit =
  ...
  | E_vector_first_where(is_par,(p,e1),e2,_) ->
      fprintf fmt "first_where%s" (if is_par then "_par" else "");
      fprintf fmt "%a %a" (pp_e ~paren:true) (E_fun(p,e1))
                          (pp_e ~paren:true) e2
  in
